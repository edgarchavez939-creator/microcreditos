...
function etiquetaEstado(c: CreditoHistorial): { txt: string; cls: string } {
  ...
  if (c.estado === 'PAGADO') return { txt: 'Crédito cancelado', cls: 'bg-slate-100 text-slate-600 ring-slate-200' };
...
export function HistorialCreditos({ clienteId }: { clienteId: number }) {
  ...
  if (isLoading) return <p className="text-sm text-slate-400">Cargando historial…</p>;
  if (!creditos || creditos.length === 0) {
    return (
      <div className="card card-pad">
        <h3 className="text-sm font-semibold text-slate-700">Historial de créditos</h3>
        <p className="mt-2 text-sm text-slate-400">Este cliente aún no tiene créditos registrados.</p>
      </div>
    );
  }
    ...
    <div className="card card-pad">
      ...
      <div className="space-y-2">
        {creditos.map((c) => {
          ...
          return (
            <div key={c.id} className={`rounded-xl ring-1 ${c.es_ultimo_pagado ? 'ring-money-200' : 'ring-slate-100'}`}>
              <div className="flex cursor-pointer flex-wrap items-center justify-between gap-2 p-3"
                ...
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="font-medium text-slate-700">{c.numero_credito ?? `Crédito #${c.id}`}</span>
                ...
                <div className="flex items-center gap-3">
                  <div className="text-right">
                    ...
                    <div className="text-xs text-slate-400">{c.plazo_meses} meses</div>
                  </div>
