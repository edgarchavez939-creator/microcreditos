/** Catálogo DANE de municipios de Colombia (código | nombre). */
const RAW = `05002|ABEJORRAL
54003|ÁBREGO
05004|ABRIAQUÍ
50006|ACACÍAS
27006|ACANDÍ
...
export const MUNICIPIOS: Municipio[] = RAW.split('\n').map((l) => {
  const [codigo, ...resto] = l.split('|');
  return { codigo, nombre: resto.join('|') };
});

/** Valor a guardar: "NOMBRE (CODIGO)". */
export function etiquetaMunicipio(m: Municipio): string {
  return `${m.nombre} (${m.codigo})`;
}

/** ¿El texto corresponde a un municipio válido del catálogo? */
export function esMunicipioValido(valor: string): boolean {
  return MUNICIPIOS.some((m) => etiquetaMunicipio(m) === valor);
}
